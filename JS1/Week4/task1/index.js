...
function query(collection) {
	var clonnedCollection = clone(collection)
	var actions = [].slice.call(arguments).slice(1)
	if (actions.length === 0) return clonnedCollection
	...
	actions.forEach(action => {
		if (action[0] === 'filterIn') {
			//console.log('filter')
			clonnedCollection = clonnedCollection.filter(item => {
				for (let i = 0; i < action[2].length; i++) {
					if (item[action[1]] === action[2][i]) return true
				}
			})
			//console.log(clonnedCollection)
		}

		if (action[0] === 'select') {
			//console.log('select')
			var arr = []
			clonnedCollection.forEach(item => {
				var obj = {}
				for (let i = 0; i < action[1].length; i++) {
					if (!item.hasOwnProperty(action[1][i])) continue
					obj[action[1][i]] = item[action[1][i]]
				}
				arr.push(obj)
			})

			clonnedCollection = arr
			//console.log(clonnedCollection)
		}
	})

	return clonnedCollection
}
